...
>
> **V16.3 interactive-filter notes:** (1) The Leadership **Financial Snapshot** has a **Domain filter** (`ls-fin-domain` → `lsRenderFin()` over the `LSFIN` per-domain cube) that re-drives its KPIs, FTE Trend & FTE Cost charts, and insight note. Financials are now multi-domain totals (Hybrid Cloud / Database / Backup / Capacity). (2) The Leadership **Incident Snapshot** has an **Assignment-group filter** (`ls-inc-group`) driving all its KPIs + the aging chart via the `LSINC` cube, plus an **Auto-resolved filter** on the MTTR chart (`ls-mttr-auto`); the MTTR trend recomputes on **group × auto** via `LSMTTR` + `lsComputeMttrTrend()` / `lsRenderIncident()`. (3) Innovation Roadmap **status + benefits come from the roadmap slide text** (e.g. statuses In Progress / Evaluating, platform per slide) — read the slide's embedded status/benefit lines, don't infer from dates.
...
- **Always keep the Leadership Summary interactive filters** — the Financial Snapshot **Domain filter** (`#ls-fin-domain` → `lsRenderFin()`) and the Incident Snapshot **Assignment-group** (`#ls-inc-group`) and **Auto-resolved** (`#ls-mttr-auto`) filters must remain present and functional. Populate each filter's `<option>` list from the data's real domains/groups (never delete the `<select>` even for a single-domain account — list the one domain plus "All"), and **call `lsRenderFin()` and `lsRenderIncident()` inside the `DOMContentLoaded` handler** (alongside `initCharts()`, which should be wrapped in try/catch) so the snapshots are synced to their selectors on load rather than only on change.
